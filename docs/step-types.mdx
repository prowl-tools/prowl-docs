...
Prowl QA supports 28 step types. Most have both a **shorthand** form (concise, readable) and an **explicit** form (full control over selectors).
...
The downloaded file is saved into the run directory (`.prowlqa/runs/<timestamp>/`).
...
- `file` (required) — path to the script; absolute, or relative to the config directory (`.prowlqa/`)
...
- `name` (required) — baseline identifier; stored at `.prowlqa/baselines/<name>.png`
...
On the first run the baseline is created and the step passes. On later runs the current screenshot is compared against the baseline; the step fails if the difference exceeds `threshold`, writing a diff image into the run directory. Accept new baselines with `prowlqa update-baselines`.
