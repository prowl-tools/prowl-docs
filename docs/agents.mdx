...
Jump to the [Library API](#library-api-nodejs) for programmatic Node.js integration, the [MCP Server](/mcp) to connect an agent without shell access, or [Using Starter Templates](#using-starter-templates) for the hunts the CLI bundles.
...
:::tip
That's the full loop — install, run, branch. The sections below cover the details: workflow patterns, the Node.js library API, CI integration, and the CLI's starter templates.
:::
...
On a **retried** run (a hunt with a [`retry`](/troubleshooting#retry-diagnostics) block where more than one attempt ran), the result also carries `retryHistory` and `retrySummary` (0.1.10). Both are **absent** on a first-attempt pass and on any artifact written before 0.1.10, so existing parsers are unaffected:

```json
{
  "retryHistory": [
    { "attempt": 1, "status": "fail", "durationMs": 5010, "failedStep": { "index": 0, "type": "navigate" }, "error": "Timeout 5000ms exceeded" },
    { "attempt": 2, "status": "pass", "durationMs": 240 }
  ],
  "retrySummary": "Passed on attempt 2 of 3 — first failure: navigate (Timeout 5000ms exceeded)"
}
```

Each `retryHistory` record has a 1-based `attempt` (attempt 1 is the initial run), a `status`, its `durationMs`, and — on a failed attempt only — the first `failedStep` (`{ index, type }`, `index` 0-based) and the `error`. The last entry always matches the final result. See [Retry diagnostics](/troubleshooting#retry-diagnostics).
...
```json
[
  { "hunt": "smoke-test", "status": "pass", "durationMs": 1240, "startedAt": "2026-05-28T17:15:12.481Z", "runDir": ".prowl/runs/2026-05-28_17-15-12-481", "retries": 1 },
  { "hunt": "smoke-test", "status": "fail", "durationMs": 1840, "startedAt": "2026-05-28T09:02:55.001Z", "runDir": ".prowl/runs/2026-05-28_09-02-55-001" }
]
```

Each entry carries an optional `retries` count (0.1.10) — retry attempts beyond the first — omitted when zero; older entries without it still parse. Retention is configurable via `history.maxRuns` — see [Configuration](/configuration#history).

## Connect Over MCP

For agents that support the [Model Context Protocol](https://modelcontextprotocol.io), Prowl ships an MCP server that exposes QA as named tools (`list_hunts`, `run_hunt`, `run_suite`, `list_projects`) over stdio — no shell access required. The agent connects once and drives runs through tool calls, with `run_suite` optionally logging failures straight to your backlog.
...
`runSuite()` runs an entire hunt suite programmatically — the same engine behind `prowl ci` — and `updateBacklogFromSuite()` turns its failures into deduplicated bug tickets. This is the non-CLI path to the same automated bug-logging the MCP [`run_suite` tool](/mcp#logging-bugs-automatically) performs.
...
`HistoryEntry` is `{ hunt: string; status: "pass" | "fail"; durationMs: number; startedAt: string; runDir?: string; retries?: number }` (`retries` added in 0.1.10 — retry attempts beyond the first, omitted when zero). Retention is capped per hunt by `history.maxRuns` — see [Configuration](/configuration#history).

## Using Starter Templates

Prowl ships starter hunts **inside the CLI**. Running [`prowl init`](/starter-templates) scaffolds a `.prowl/` directory with four ready-to-edit hunts — `hello.yml` (a minimal smoke test), `login-flow.yml` (a commented auth example), `form.yml` (a forms example), and `macos-hello.yml` (a desktop-first macOS starter, experimental) — so an agent has a concrete pattern to copy without fetching from any external registry.

### Manual Workflow

1. Run `prowl init` to scaffold the [starter hunts](/starter-templates).
2. Copy a starter to a new file in `.prowl/hunts/` — the file name is the hunt's identity.
3. Customize `{{VAR}}` placeholders and selectors for your environment.
...
### Generating hunts from scratch

An agent can also author hunt YAML directly and validate it with the exported `huntSchema` before running — no template needed. See [Schemas](#schemas) above.

### Library API for Templates

Use the library API to customize and run a starter (or any hunt) programmatically:
...
:::note
The starter hunts ship inside the CLI. Contribute your own by opening a pull request to the [`prowl`](https://github.com/prowl-tools/prowl) repository — see [Starter Templates](/starter-templates#contributing-a-template).
:::

<div className="card-grid">
  <a className="card" href="/starter-templates">
    <h3>Starter Templates</h3>
    <p>The hunts prowl init bundles, and how to contribute one</p>
  </a>
