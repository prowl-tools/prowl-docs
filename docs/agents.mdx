...
Prowl ships starter hunts **inside the CLI**. Running [`prowl init`](/starter-templates) scaffolds a `.prowl/` directory with two ready-to-edit hunts — `hello.yml` (a minimal smoke test) and `login-flow.yml` (a commented auth example) — so an agent has a concrete pattern to copy without fetching from any external registry.
