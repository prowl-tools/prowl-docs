...
On a **retried** run (a hunt with a [`retry`](/getting-started) block where more than one attempt ran), the result also carries `retryHistory` and `retrySummary` (0.1.10). Both are **absent** on a first-attempt pass and on any artifact written before 0.1.10, so existing parsers are unaffected:
...
```json
{
  ...
  "retrySummary": "Passed on attempt 2 of 3 — first failure: navigate (timeout)"
}
```
